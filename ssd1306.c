...
void ssd1306_command(uint8_t c)
{
    if (use_i2c) {
        ...
        APP_ERROR_CHECK(ret);
	    SEGGER_RTT_printf(0,"I2C Tranfer done for command: %x\n", c);
        UNUSED_VARIABLE(ret);
    }
    else {
        _HI_CS();
...
{
    SEGGER_RTT_WriteString(0,"Being ssd1306 init sequence\n");
    _vccstate = vccstate;
    _i2caddr = i2caddr;
...
/*    ssd1306_command(SSD1306_SETLOWCOLUMN | 0x0);  // low col = 0
    ...
    ssd1306_command(0x7F); // Column end address (127 = reset)

    ssd1306_command(SSD1306_PAGEADDR);
    ssd1306_command(0); // Page start address (0 = reset)
    ssd1306_command(3); // Page end address for 128x32 */
/*#if SSD1306_LCDHEIGHT == 64
    ...
    if (use_i2c) {
        ...
        uint8_t data_chunk [17];
        data_chunk[0] = control;

        for (uint8_t i=0; i<32; i++) {
            for (uint16_t j=0; j< 16; j++) {
                // data_chunk[1+j] = buffer[(32*(32 - i%32)) + j];
                data_chunk[1+j] = buffer[32*i + j];
            }
            err_code = nrf_drv_twi_tx(&m_twi_master, _i2caddr, data_chunk, (uint8_t) (sizeof(data_chunk)) , false);
            APP_ERROR_CHECK(err_code);

        }
        // err_code = nrf_drv_twi_tx(&m_twi_master, _i2caddr, &control, 3)]
        // err_code = nrf_drv_twi_tx(&m_twi_master, _i2caddr, &control, 1, true);
        // APP_ERROR_CHECK(err_code);
        // SEGGER_RTT_WriteString(0,"Sent control 0x40\n");
        // err_code = nrf_drv_twi_tx(&m_twi_master, _i2caddr, buffer, (uint8_t) (SSD1306_LCDWIDTH * SSD1306_LCDHEIGHT / 8) , true);
        // APP_ERROR_CHECK(err_code);
        // nrf_drv_twi_xfer_desc_t xfer_desc =
            // NRF_DRV_TWI_XFER_DESC_TX(_i2caddr, buffer, (uint8_t) (SSD1306_LCDWIDTH * SSD1306_LCDHEIGHT / 8));
            // NRF_DRV_TWI_XFER_DESC_TX(_i2caddr, buffer, (uint8_t) (128));
        // err_code = nrf_drv_twi_xfer(&m_twi_master, &xfer_desc, NRF_DRV_TWI_FLAG_TX_POSTINC);
        // APP_ERROR_CHECK(err_code);

        /*
        for (uint16_t i = 0; i < (SSD1306_LCDWIDTH * SSD1306_LCDHEIGHT / 8) - 1; i++) {
                err_code = nrf_drv_twi_tx(&m_twi_master, _i2caddr, buffer +  i, (uint8_t) (1), true);
                APP_ERROR_CHECK(err_code);
        }
        */
/*            SEGGER_RTT_WriteString(0,"Sending last data\n");
            err_code = nrf_drv_twi_tx(&m_twi_master, _i2caddr, buffer - 1 + (SSD1306_LCDWIDTH *                                                     SSD1306_LCDHEIGHT / 8), (uint8_t) (1), false);
            APP_ERROR_CHECK(err_code);
            SEGGER_RTT_WriteString(0,"Sent data\n");*/
    }
    else {
        _HI_CS();
        _HI_DC();
