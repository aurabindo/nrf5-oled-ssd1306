 ...
 */
static void spi_master_event_handler(const nrf_drv_spi_evt_t *event)
{
    switch (event->type) {
                   ...
                   const uint16_t len)
{
    ...
    // Start transfer.
    //nrf_drv_spi_transfer(&m_spi_master,p_tx_data, len, p_rx_data, len);
